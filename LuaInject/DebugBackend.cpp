...
extern cocos_drawScene_t                cocos_drawScene_dll;
extern cocos_pause_t                    cocos_pause_dll;
extern cocos_resume_t                   cocos_resume_dll;
extern int		cocosCurDrawCallIndex;
extern int		cocosCurDrawCallSetIndex;
extern int		cocosCurDrawCallMax  ;
...
DebugBackend::DebugBackend()
{
	...
	m_isLuaProfilerMode     = false;
	m_hasHookCocos = false;
	m_eventChannel.setLogName("c:\\temp\\event_b.txt");
	m_commandChannel.setLogName("c:\\temp\\command_b.txt");
	m_NameList = new stdext::hash_map<unsigned int, std::string>();
...
void DebugBackend::cocosDrawSceneTriggle()
{
	if (m_isLuaProfilerMode)
	{
		LuaProfilerBackend::Get()->TriggleOneLuaFrame();
	}
...
void DebugBackend::HookCallback(unsigned long api, lua_State* L, lua_Debug* ar)
{
	...
	if (m_isLuaProfilerMode && (event == LUA_HOOKCALL || event == LUA_HOOKRET))
	{

		lua_getinfo_dll(api, L, "nS", ar);
		...
		LuaPF OneFunc = { 0 };
		OneFunc.CMD = event;
		OneFunc.Line = GetLastLineDefined(api, ar);
		int Line = GetLineDefined(api, ar);
		int Line2 = GetCurrentLine(api, ar);
		const char* source = GetSource(api, ar);
		const char* name = GetName(api, ar);

		unsigned int source1 = (unsigned int)source;
		unsigned int name1 = (unsigned int)name;
		if (source != (const char*)source1)
		{
			assert(false);
		}
		if (name != (const char*)name1)
		{
			assert(false);
		}
		
		if (source1 !=0 && m_NameList->find(source1) == m_NameList->end())
		{
			(*m_NameList)[source1] = std::string(source);
		}
		if (name1 != 0 &&   m_NameList->find(name1) == m_NameList->end())
		{
			(*m_NameList)[name1] = std::string(name);
		}


		OneFunc.FileNameID			= (unsigned int)source;
		OneFunc.FuncId				= (unsigned int)name;
		OneFunc.CurrentMem			= 0;
		OneFunc.CureentTime			= _highp_clock();
		LuaProfilerBackend::Get()->AddOnePF(OneFunc);
...
void DebugBackend::InstallHookCocosGame()
{
	if (!m_hasHookCocos)
		InstallCocosHooker();
}
...
void DebugBackend::PauseCocosGame()
{
	if (cocos_cocos_pause_stdcall)
	{
		cocos_cocos_pause_stdcall();
	}
}

void DebugBackend::ResumeCocosGame()
{
	if (cocos_cocos_resume_stdcall)
	{
		cocos_cocos_resume_stdcall();
	}
}

void DebugBackend::SetCocosDrawCallIndex(int index)
{
	cocosCurDrawCallSetIndex = index;
}

 

void DebugBackend::SendCocosDrawCallMax()
{
	m_eventChannel.WriteUInt32(EventId_GetDrawCallMax);
	m_eventChannel.WriteUInt32(0); 
	m_eventChannel.WriteUInt32(cocosCurDrawCallMax);
	m_eventChannel.WriteUInt32(cocosCurDrawCallIndex);
	m_eventChannel.Flush();
}
...
void DebugBackend::CommandThreadProc()
{
    ...
    while (m_commandChannel.ReadUInt32(commandId))
    {
		...
		else if (commandId == CommandId_OpenSocketChannal)
		{
			unsigned int port;
			m_commandChannel.ReadUInt32(port);
			LuaProfilerBackend::Get()->ConnectClient(port);
		}
		...
		else if (commandId == CommandId_StartProfiler)
		{
			 
			m_isLuaProfilerMode = true;
			ActiveLuaProfilerInAllVms();
			
		 
		}
		else if (commandId == CommandId_StopProfiler)
		{
			m_isLuaProfilerMode = false;
			ActiveLuaHookInAllVms();
			Continue();
		}
		else if (commandId == CommandId_PauseCocosGame)
		{
			PauseCocosGame();
		}
		else if (commandId == CommandId_ResumeCocosGame)
		{
			ResumeCocosGame();
		}
		else if (commandId == CommandId_InstallCocosHook)
		{
			InstallHookCocosGame();
		}
		else if (commandId == CommandId_SetDrawCallIndex)
		{
			unsigned int index;
			m_commandChannel.ReadUInt32(index);
			SetCocosDrawCallIndex(index);
		}
		else if (commandId == CommandId_GetDrawCallMax)
		{
			SendCocosDrawCallMax();
		}

		else if (commandId == CommandId_ReqFuncOrFileName)
		{
			...
			m_commandChannel.ReadUInt32(namePtr);
			std::string strPtr = "null";
			stdext::hash_map<unsigned int, std::string>::iterator iter = m_NameList->find(namePtr);
			if (iter == m_NameList->end())
			{
				strPtr = iter->second;
			}
				m_eventChannel.WriteUInt32(EventId_LuaFuncOrFileName);
				m_eventChannel.WriteUInt32(vm);
				m_eventChannel.WriteUInt32(namePtr);
				m_eventChannel.WriteString(strPtr);
				m_eventChannel.WriteLog(strPtr.c_str());
...
void DebugBackend::ActiveLuaProfilerInAllVms()
{
	StateToVmMap::iterator end = m_stateToVm.end();
	...
	for (StateToVmMap::iterator it = m_stateToVm.begin(); it != end; it++)
	{
		VirtualMachine* vm = it->second;
		//May have issues with L not being the currently running thread
		SetHookMode(vm->api, vm->L, HookMode_CallsAndReturns);
	}
}
