...
typedef void*      (*drawScene_cdecl_t)         ();
...
bool LoadLuaFunctionsFrom_Lua51DLL( )
{
#define GET_FUNCTION_OPTIONAL51(function)                                                                                     \
        {                                                                                                                       \
            FARPROC luaFunc =  GetProcAddress(hModuleLua51, #function);															 \
            if (luaFunc)                                                                                      \
            {                                                                                                                   \
                luaInterface.function##_dll_cdecl = reinterpret_cast<function##_cdecl_t>(luaFunc);                     \
            }                                                                                                                   \
        }

#define GET_FUNCTION51(function)                                                                                              \
        GET_FUNCTION_OPTIONAL51(function)                                                                                         \
...
#define HOOK_FUNCTION51(function)                                                                                             \
        if (luaInterface.function##_dll_cdecl != NULL)                                                                          \
        {                                                                                                                       \
	...
	HMODULE hModuleLua51 = GetModuleHandle("lua51.dll");
	if (hModuleLua51 == NULL)
		return false;
	...
	GET_FUNCTION_OPTIONAL51(lua_open);
	if (luaInterface.lua_open_dll_cdecl == NULL)
	{
		GET_FUNCTION51(lua_newstate);
	}

	// Start reporting errors about functions we couldn't hook.
	report = true;

	GET_FUNCTION51(lua_newthread);
	GET_FUNCTION51(lua_close);
	GET_FUNCTION51(lua_error);
	GET_FUNCTION_OPTIONAL51(lua_absindex); // Only present in Lua 5.2+
	GET_FUNCTION51(lua_sethook);
	GET_FUNCTION51(lua_getinfo);
	GET_FUNCTION51(lua_remove);
	GET_FUNCTION51(lua_settable);
	GET_FUNCTION51(lua_gettable);
	GET_FUNCTION51(lua_rawget);
	GET_FUNCTION51(lua_rawgeti);
	GET_FUNCTION51(lua_rawset);
	GET_FUNCTION51(lua_pushstring);
	GET_FUNCTION51(lua_pushlstring);
	GET_FUNCTION51(lua_type);
	GET_FUNCTION51(lua_typename);
	GET_FUNCTION51(lua_settop);
	GET_FUNCTION51(lua_gettop);
	GET_FUNCTION51(lua_getlocal);
	GET_FUNCTION51(lua_setlocal);
	GET_FUNCTION51(lua_getstack);
	GET_FUNCTION51(lua_insert);
	GET_FUNCTION51(lua_pushnil);
	GET_FUNCTION51(lua_pushvalue);
	GET_FUNCTION51(lua_pushcclosure);
	GET_FUNCTION51(lua_pushnumber);
	GET_FUNCTION51(lua_pushlightuserdata);
	GET_FUNCTION51(lua_checkstack);
	GET_FUNCTION51(lua_gethookmask);

	// Only present in Lua 5.2+
	GET_FUNCTION_OPTIONAL51(lua_getglobal);
	GET_FUNCTION_OPTIONAL51(lua_setglobal);

	// Only present in Lua 5.1 (*number funtions used in Lua 4.0)
	GET_FUNCTION_OPTIONAL51(lua_pushinteger);
	GET_FUNCTION_OPTIONAL51(lua_tointeger);
	GET_FUNCTION_OPTIONAL51(lua_tointegerx);

	// Only present in Lua 4.0 and 5.0 (exists as a macro in Lua 5.1)
	GET_FUNCTION_OPTIONAL51(lua_tostring);

	if (luaInterface.lua_tostring_dll_cdecl == NULL)
	{
		GET_FUNCTION51(lua_tolstring);
	}

	GET_FUNCTION_OPTIONAL51(lua_tonumberx);

	if (luaInterface.lua_tonumberx_dll_cdecl == NULL)
	{
		// If the Lua 5.2 tonumber isn't present, require the previous version.
		GET_FUNCTION51(lua_tonumber);
	}

	GET_FUNCTION51(lua_toboolean);
	GET_FUNCTION51(lua_tocfunction);
	GET_FUNCTION51(lua_touserdata);

	// Exists as a macro in Lua 5.2
	GET_FUNCTION_OPTIONAL51(lua_callk);
	if (luaInterface.lua_callk_dll_cdecl == NULL)
	{
		GET_FUNCTION51(lua_call);
	}

	// Exists as a macro in Lua 5.2
	GET_FUNCTION_OPTIONAL51(lua_pcallk);
	if (luaInterface.lua_pcallk_dll_cdecl == NULL)
	{
		GET_FUNCTION51(lua_pcall);
	}

	// Only present in Lua 4.0 and 5.0 (exists as a macro in Lua 5.1)
	GET_FUNCTION_OPTIONAL51(lua_newtable);
	if (luaInterface.lua_newtable_dll_cdecl == NULL)
	{
		GET_FUNCTION51(lua_createtable);
	}

	GET_FUNCTION51(lua_load);
	GET_FUNCTION51(lua_next);
	GET_FUNCTION51(lua_rawequal);
	GET_FUNCTION51(lua_getmetatable);
	GET_FUNCTION51(lua_setmetatable);
	GET_FUNCTION_OPTIONAL51(luaL_ref);
	GET_FUNCTION_OPTIONAL51(luaL_unref);
	GET_FUNCTION51(luaL_newmetatable);
	GET_FUNCTION51(lua_getupvalue);
	GET_FUNCTION51(lua_setupvalue);

	// Only available in Lua 5.0/5.1
	GET_FUNCTION_OPTIONAL51(lua_getfenv);
	GET_FUNCTION_OPTIONAL51(lua_setfenv);

	if (luaInterface.version >= 510)
	{
		GET_FUNCTION51(lua_pushthread);
	}
	else
	{
		// This function doesn't exist in Lua 5.0, so make it optional.
		GET_FUNCTION_OPTIONAL51(lua_pushthread);
	}

	GET_FUNCTION51(lua_newuserdata);

	// This function isn't strictly necessary. We only hook it
	// in case the base function was inlined.
	GET_FUNCTION_OPTIONAL51(luaL_newstate);
	GET_FUNCTION_OPTIONAL51(luaL_loadbuffer);
	GET_FUNCTION_OPTIONAL51(luaL_loadfile);
	GET_FUNCTION_OPTIONAL51(luaL_loadfilex);

	// These functions only exists in LuaPlus.
	GET_FUNCTION_OPTIONAL51(lua_towstring);
	GET_FUNCTION_OPTIONAL51(lua_iswstring);
	...
	HOOK_FUNCTION51(lua_open);
	HOOK_FUNCTION51(lua_open_500);
	HOOK_FUNCTION51(lua_newstate);
	HOOK_FUNCTION51(lua_close);
	HOOK_FUNCTION51(lua_newthread);
	HOOK_FUNCTION51(lua_pcall);
	HOOK_FUNCTION51(lua_pcallk);
	HOOK_FUNCTION51(lua_call);
	HOOK_FUNCTION51(lua_callk);
	HOOK_FUNCTION51(lua_load);
	HOOK_FUNCTION51(lua_load_510);
	HOOK_FUNCTION51(luaL_newmetatable);
	HOOK_FUNCTION51(lua_sethook);

	HOOK_FUNCTION51(luaL_loadbuffer);
	HOOK_FUNCTION51(luaL_loadfile);
	HOOK_FUNCTION51(luaL_loadfilex);
	HOOK_FUNCTION51(luaL_newstate);
...
void PostLoadLibrary(HMODULE hModule)
{
    ...
    if (g_loadedModules.find(moduleName) == g_loadedModules.end())
    {
		...
		if (!LoadLuaFunctionsFrom_Lua51DLL())
		{
