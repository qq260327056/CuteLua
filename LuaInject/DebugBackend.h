...
public:
	...
	void ActiveLuaProfilerInAllVms();
...
private:
	...
	// only for cocos game;
	void PauseCocosGame();
	void ResumeCocosGame();
	void SetCocosDrawCallIndex(int index);
	void SendCocosDrawCallMax();
	void InstallHookCocosGame();
...
private:
	...
	bool							m_isLuaProfilerMode;
	bool							m_hasHookCocos;
	stdext::hash_map<unsigned int, std::string>*		m_NameList;
};
